...
for (cycle_times in 1:All_cycle_times) {
  ...
  ob_test_pred<-predict(ob_pred_model,
                        test_frame[,1:ncol(test_frame)],
                        type="probabilities")
  #模型评价
  ROC_frame<-data.frame(pre=as.numeric(ob_test_pred[,which(colnames(ob_test_pred)=='1')]),
  ...
  ENPEP_Result<-predict(ob_pred_model,
                        ENPEP_data,
                        type="response")
  ENPEP_Result<-as.vector(ENPEP_Result)
  #结果写入表格
  jishu<-jishu+1
  OB_Model_result_frame[jishu,]<-c(numbers_to_char(c(OB_rows,N_rows)),
                                   numbers_to_char(as.numeric(ob_test_pred[,which(colnames(ob_test_pred)=='1')])),
                                   numbers_to_char(as.vector(ob_test_label)),
                                   as.numeric(p1$auc),
                                   length(which(ob_train_label=='1')),
                                   length(which(ob_train_label=='0')),
                                   cor(as.numeric(ob_test_pred[,which(colnames(ob_test_pred)=='1')]),
                                       as.numeric(as.vector(ob_test_label))),
                                   MAE(as.numeric(as.vector(ob_test_label)),
                                       as.numeric(ob_test_pred[,which(colnames(ob_test_pred)=='1')])),
                                   as.vector(ENPEP_Result))
  #pb$tick()  # 更新进度条
}

#D.输出训练结果
write.csv(OB_Model_result_frame,'OB_Model_result_frame_SVM.csv')
