...
for (cycle_times in 1:All_cycle_times) {
  ...
  train_frame$Label<-as.numeric(train_frame$Label)
  ...
  #建立随机森林模型
  ob_pred_model<-randomForest(Label~., data = train_frame, importance = TRUE)
  #进行预测
  ob_test_pred<-predict(ob_pred_model,
                        test_frame)
  ROC_frame<-data.frame(pre=as.numeric(ob_test_pred),
                        answer=as.numeric(ob_test_label))
  p1<-roc(answer ~ pre, ROC_frame,levels = c(0,1),
          direction='<')
  #检测ENPEP结果
  ENPEP_Result<-predict(ob_pred_model,
                        ENPEP_data,type='class')
  #保存模型
  RF_ob_pred_model<-ob_pred_model
  save(RF_ob_pred_model, file = paste0('single_RF_models/RF_',temp_cycle_times,'.RData'))
  #结果写入表格
  jishu<-jishu+1
  OB_Model_result_frame[jishu,]<-c(numbers_to_char(c(OB_rows,N_rows)),
                                   numbers_to_char(as.numeric(ob_test_pred)),
                                   numbers_to_char(as.vector(ob_test_label)),
                                   as.numeric(p1$auc),
                                   length(which(ob_train_label==1)),
                                   length(which(ob_train_label==0)),
                                   cor(ob_test_pred,
                                       as.numeric(ob_test_label)),
                                   MAE(ob_test_pred,
                                       ob_test_pred),
                                   as.character(ENPEP_Result))
  pb$tick()  # 更新进度条
}


#D.输出训练结果
write.csv(OB_Model_result_frame,'OB_Model_result_frame_RF.csv')
